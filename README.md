# Linux.do 小助手

![License](https://img.shields.io/badge/license-MIT-blue.svg)
![Version](https://img.shields.io/badge/version-v1.0.1-green.svg)
![Chrome](https://img.shields.io/badge/Chrome-Manifest%20V3-brightgreen.svg)
![Stars](https://img.shields.io/github/stars/xiaohuihui202504/linuxdo-helper-extension.svg)

> Linux.do 论坛增强工具 — 自动浏览、智能点赞、账号等级追踪、积分查看、排行榜，让论坛体验更高效

## 项目介绍

Linux.do 小助手是一款专为 [linux.do](https://linux.do) 论坛设计的浏览器扩展（Chrome / Edge / Arc 等 Chromium 内核浏览器），集成自动浏览、点赞计数与配额管理、账号等级追踪、CREDIT 积分查看、CDK 分数查询、排行榜展示、用户信息速览、界面美化等功能，帮助用户高效使用论坛、轻松管理账号等级。

### 核心亮点

- 模拟人类浏览行为的智能自动滚动
- 基于信任等级的点赞配额精准追踪
- 账号信任等级进度可视化（对接 connect.linux.do）
- CREDIT 积分实时查看与统计
- CDK 社区分数查询
- 多维度排行榜展示（日/周/月/季/年/总榜）
- 一键查看帖子作者详细信息
- 可折叠面板区块，数据懒加载
- 多主题配色 + 简洁模式 + 中英双语

## 功能清单

| 功能模块 | 功能说明 | 状态 |
|---------|---------|------|
| 自动浏览 | 智能滚动阅读，支持未读优先、随机顺序、跳过已读、限定篇数 | ✅ |
| 阅读统计 | 本次/今日/累计阅读数据统计 | ✅ |
| 滑块控制 | 获取数量、休息时间、阅读限制均支持滑块调节 | ✅ |
| 点赞计数 | 24 小时点赞实时追踪，按信任等级显示配额（Lv0-Lv4: 50-150） | ✅ |
| 冷却倒计时 | 触发频率限制时显示精确冷却倒计时 | ✅ |
| 自动点赞 | 浏览时自动为主贴点赞，可开启回复快速点赞（最多 5 条） | ✅ |
| 点赞过滤 | 阈值模式（最低点赞数）/ 概率模式（基于热度）/ 分区白名单黑名单 | ✅ |
| 账号信息 | 展示当前信任等级，可视化升级进度条（对接 connect.linux.do） | ✅ |
| CREDIT 积分 | 查看可用积分、明日积分、当前点数、排名、近 7 天收入等（对接 credit.linux.do） | ✅ |
| CDK 分数 | 查看 CDK 社区总分（对接 cdk.linux.do） | ✅ |
| 排行榜 | 日榜/周榜/月榜/季榜/年榜/总榜积分与排名展示 | ✅ |
| 用户信息 | 悬浮查看用户注册天数、发帖数、回复数、信任等级 | ✅ |
| 批量查看 | 一键加载当前页面所有用户信息 | ✅ |
| 折叠面板 | 各功能区块可折叠/展开，状态自动保存，数据懒加载 | ✅ |
| 暂停按钮 | 面板标题栏快捷暂停/恢复阅读 | ✅ |
| CF 5秒盾 | CloudFlare 5秒盾检测失败时自动跳转验证页面，支持手动触发 | ✅ |
| 简洁模式 | 隐藏侧边栏和导航栏，专注阅读 | ✅ |
| 灰度模式 | 页面灰度化显示 | ✅ |
| 主题配色 | 6 种配色方案：紫罗兰 / 海洋蓝 / 森林绿 / 暖阳橙 / 樱花粉 / 暗夜黑 | ✅ |
| 多语言 | 支持中文和英文界面切换 | ✅ |
| 随机楼层 | 随机跳转到帖子中的某一楼 | ✅ |

## 安装说明

### 环境要求

- Chrome 88+ / Edge 88+ / Arc 或其他 Chromium 内核浏览器
- 支持 Manifest V3 的浏览器版本

### 安装步骤

1. 下载项目代码：

```bash
git clone https://github.com/xiaohuihui202504/linuxdo-helper-extension.git
```

2. 打开 Chrome 浏览器，进入扩展管理页面：

```
chrome://extensions/
```

3. 开启右上角的 **开发者模式**

   ![image-20260211172137027](https://mypicture-1258720957.cos.ap-nanjing.myqcloud.com/image-20260211172137027.png)

4. 点击 **加载已解压的扩展程序**，选择项目根目录

   ![image-20260211172304060](https://mypicture-1258720957.cos.ap-nanjing.myqcloud.com/image-20260211172304060.png)

   ![image-20260211172329726](https://mypicture-1258720957.cos.ap-nanjing.myqcloud.com/image-20260211172329726.png)

5. 扩展安装完成，访问 [linux.do](https://linux.do) 即可看到右上角的浮动面板

   ![img](https://mypicture-1258720957.cos.ap-nanjing.myqcloud.com/517133078f3ebaf5233ea5e107256aa9.png)

> 同样适用于 Edge、Arc 等 Chromium 内核浏览器。Edge 的扩展管理页面为 `edge://extensions/`。

## 使用说明

### 快速开始

1. 安装扩展后，访问 [linux.do](https://linux.do) 论坛
2. 页面右上角会出现浮动控制面板
3. 通过面板中的开关控制各项功能

### 功能操作

| 操作 | 说明 |
|------|------|
| 开启自动浏览 | 打开面板中的「自动浏览」区块，点击开始阅读按钮 |
| 暂停/恢复阅读 | 面板标题栏暂停按钮，快捷切换 |
| 配置浏览策略 | 可选：仅未读、随机顺序、跳过已读、限定篇数 |
| 滑块调节 | 拖动滑块调节获取数量、休息时间、阅读限制数量 |
| 开启自动点赞 | 打开「自动点赞」开关，浏览时自动为主贴点赞 |
| 点赞过滤 | 选择阈值模式或概率模式，控制点赞策略 |
| 查看账号信息 | 展开「账号信息」区块，查看信任等级及升级进度 |
| 查看 CREDIT 积分 | 展开「CREDIT 积分」区块，查看积分详情（需登录 credit.linux.do） |
| 查看 CDK 分数 | 展开「CDK 分数」区块，查看社区分数（需登录 cdk.linux.do） |
| 查看排行榜 | 展开「排行榜」区块，查看日/周/月/季/年/总榜排名 |
| 查看用户信息 | 帖子旁悬浮按钮，点击加载用户详情 |
| 切换主题 | 「插件设置」区块中选择配色方案 |
| 简洁模式 | 开启后隐藏论坛侧边栏和导航栏 |
| CF 5秒盾 | 「插件设置」中开启，自动处理 CloudFlare 验证 |
| 查看统计 | 点击浏览器工具栏扩展图标，查看阅读和点赞统计 |

### 信任等级与点赞配额

| 信任等级 | 每日点赞上限 |
|---------|-------------|
| Lv0 | 50 |
| Lv1 | 50 |
| Lv2 | 75 |
| Lv3 | 100 |
| Lv4 | 150 |

## 项目结构

```
linuxdo-helper-extension/
├── manifest.json          # 扩展配置（Manifest V3）
├── popup.html             # 弹出页面
├── popup.js               # 弹出页面逻辑
├── css/
│   └── panel.css          # 浮动面板样式
├── js/
│   ├── content.js         # 内容脚本入口
│   ├── background.js      # Service Worker（含 API 代理）
│   ├── utils.js           # 工具函数与配置常量
│   ├── storage.js         # 存储抽象层（Chrome Storage + localStorage）
│   ├── likeCounter.js     # 点赞计数、配额管理与服务器同步
│   ├── userInfo.js        # 用户信息获取与展示
│   ├── autoScroll.js      # 自动浏览控制器
│   ├── dataModules.js     # 数据模块（账号信息、CREDIT 积分、CDK 分数、排行榜）
│   └── ui.js              # 浮动面板 UI（折叠区块、懒加载）
└── icons/                 # 扩展图标（16/32/48/128px）
```

## 技术栈

| 技术 | 版本 | 用途 |
|------|------|------|
| Chrome Extension Manifest | V3 | 扩展规范 |
| JavaScript | ES6+ | 核心逻辑 |
| CSS3 | - | UI 样式（渐变、动画、Grid 布局） |
| Chrome Storage API | - | 数据持久化 |
| Chrome Runtime Message | - | 内容脚本与 Service Worker 通信、API 跨域代理 |
| Discourse API | - | 论坛数据交互 |
| Credit / CDK API | - | 积分与社区分数数据 |

## 开发指南

### 本地开发

```bash
# 克隆项目
git clone https://github.com/xiaohuihui202504/linuxdo-helper-extension.git

# 在 Chrome 中加载扩展（开发者模式）
# chrome://extensions/ → 加载已解压的扩展程序 → 选择项目目录

# 修改代码后，在扩展管理页面点击刷新按钮即可生效
```

### 模块说明

| 模块 | 文件 | 职责 |
|------|------|------|
| 工具层 | `utils.js` | DOM 操作、随机化、页面检测、全局配置常量、国际化文本 |
| 存储层 | `storage.js` | Chrome Storage API 封装，localStorage 回退 |
| 点赞模块 | `likeCounter.js` | 拦截请求追踪点赞、24 小时配额管理、服务器同步 |
| 用户模块 | `userInfo.js` | 多策略获取用户信息、缓存、批量加载 |
| 浏览模块 | `autoScroll.js` | 智能滚动、话题导航、阅读统计 |
| 数据模块 | `dataModules.js` | 账号信息、CREDIT 积分、CDK 分数、排行榜数据获取与缓存 |
| UI 模块 | `ui.js` | 浮动面板、折叠区块、滑块控件、主题切换、多语言、交互控制 |
| 入口 | `content.js` | 内容脚本初始化，模块组装 |
| 后台 | `background.js` | Service Worker 生命周期管理、API 跨域代理 |

### 贡献指南

1. Fork 本仓库
2. 创建特性分支：`git checkout -b feature/your-feature`
3. 提交更改：`git commit -m "feat: add your feature"`
4. 推送分支：`git push origin feature/your-feature`
5. 提交 Pull Request

## 常见问题

<details>
<summary>扩展安装后页面没有出现浮动面板？</summary>

1. 确认已访问 `linux.do` 域名下的页面
2. 在 `chrome://extensions/` 检查扩展是否已启用
3. 尝试刷新页面或重新加载扩展

</details>

<details>
<summary>点赞计数不准确怎么办？</summary>

点击面板中的「同步」按钮手动与服务器同步点赞数据。扩展每 30 分钟也会自动同步一次。

</details>

<details>
<summary>自动浏览时页面卡在某个位置不动了？</summary>

扩展内置了超时检测机制，正常情况下会自动恢复。如果长时间未恢复，可以手动关闭再开启自动浏览开关。

</details>

<details>
<summary>支持 Firefox 吗？</summary>

目前仅支持 Chromium 内核浏览器（Chrome、Edge、Arc 等），暂不支持 Firefox。

</details>

## 支持的站点

| 站点 | 地址 |
|------|------|
| Linux.do 主站 | `https://linux.do` |
| Connect | `https://connect.linux.do` |
| Credit | `https://credit.linux.do` |
| CDK | `https://cdk.linux.do` |

## 路线图

### 计划功能

- [ ] Firefox 浏览器支持
- [ ] 扩展商店发布（Chrome Web Store）
- [ ] 数据导出与统计报表
- [ ] 自定义快捷键
- [ ] 分区白名单/黑名单管理界面

### 已完成（v1.0.1）

- [x] 账号信任等级可视化（connect.linux.do 集成）
- [x] CREDIT 积分查看（credit.linux.do 集成）
- [x] CDK 社区分数查看（cdk.linux.do 集成）
- [x] 多维度排行榜展示
- [x] 面板折叠区块与数据懒加载
- [x] 滑块控件（获取数量、休息时间）
- [x] CloudFlare 5秒盾自动处理
- [x] Background API 跨域代理

### 优化项

- [ ] 用户信息缓存优化
- [x] 面板 UI 响应式适配（max-height: 70vh）
- [ ] 点赞策略更多自定义选项

## 更新日志

### v1.0.1

- **新增** 账号信息面板：展示当前信任等级，可视化升级进度条，对接 connect.linux.do 获取详细等级要求
- **新增** CREDIT 积分模块：查看可用积分、明日积分、当前点数、排名、近 7 天收入等数据
- **新增** CDK 分数模块：查看 CDK 社区总分，支持登录授权引导
- **新增** 排行榜模块：支持日榜/周榜/月榜/季榜/年榜/总榜，显示积分与排名
- **新增** 面板折叠区块：各功能区块可独立折叠/展开，状态自动保存
- **新增** 数据懒加载：展开区块时才请求数据，减少不必要的网络请求
- **新增** 滑块控件：获取数量、休息时间、阅读限制均改为滑块调节
- **新增** 当前阅读状态显示：实时展示阅读模式、剩余数量、今日阅读
- **新增** 暂停按钮：面板标题栏快捷暂停/恢复阅读
- **新增** CloudFlare 5秒盾处理：自动跳转验证页面，支持手动触发
- **新增** 下载位置提示：展示浏览器默认下载路径信息
- **新增** 数据模块（`dataModules.js`）：统一管理账号信息、积分、CDK、排行榜数据获取与缓存
- **新增** Background API 代理：通过 Service Worker 代理跨域请求（credit.linux.do、cdk.linux.do）
- **优化** 主题按钮显示主题名称标签
- **优化** 面板最大高度改为 70vh，适配不同屏幕
- **优化** 国际化文本大幅扩展，覆盖所有新增模块

### v1.0.0

- 初始版本发布
- 自动浏览、点赞计数、用户信息、简洁模式、主题配色、多语言等基础功能

## 致谢

- [linux.do](https://linux.do) — 提供优秀的中文技术社区平台
- [Discourse](https://www.discourse.org/) — 开源论坛框架
- 所有为本项目提出建议和反馈的用户

## 技术交流群

欢迎加入技术交流群，分享使用心得和功能建议：

![技术交流群](https://mypicture-1258720957.cos.ap-nanjing.myqcloud.com/Obsidian/image-20260217095105519.png)

## 联系作者

- **微信**: laohaibao2025
- **邮箱**: 75271002@qq.com

![微信二维码](https://mypicture-1258720957.cos.ap-nanjing.myqcloud.com/Screenshot_20260123_095617_com.tencent.mm.jpg)

## 打赏

如果这个项目对你有帮助，欢迎请作者喝杯咖啡 ☕

**微信支付**

![微信支付](https://mypicture-1258720957.cos.ap-nanjing.myqcloud.com/Obsidian/image-20250914152855543.png)

## License

本项目基于 [MIT License](LICENSE) 开源。

## Star History

如果觉得项目不错，欢迎点个 Star ⭐

[![Star History Chart](https://api.star-history.com/svg?repos=xiaohuihui202504/linuxdo-helper-extension&type=Date)](https://star-history.com/#xiaohuihui202504/linuxdo-helper-extension&Date)
